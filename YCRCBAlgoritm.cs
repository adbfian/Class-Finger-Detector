...
{
    class YCRCBAlgoritm:IColorAbstrak
    {
        //Class eksekusi Ycrb ke algoritma
        public override Image<Gray, byte> DetectSkin(Image<Bgr, byte> Img, IColor min, IColor max)
        {
            Image<Ycc, Byte> cureentYCrCbFrame = Img.Convert<Ycc, Byte>();
            Image<Gray, byte> skin = new Image<Gray, byte>(Img.Width, Img.Height);
            int y, cr, cb, x1, y1, value;
            int rows = Img.Rows;
            int cols = Img.Cols;

            Byte[, ,] YCrCBData = cureentYCrCbFrame.Data;
